...
specMain = hspec $ do
    describe "CF" $ do
        ...
        it "distanceWith euclidean should correctly compute" $ do
            (distanceWith euclidean samplesForAngelica samplesForBill) `shouldBe` 4.3
            
        it "distanceWith manhattan should result in 0 for no ratings in common" $ do
            (distanceWith manhattan M.empty M.empty) `shouldBe` 0.0
            (distanceWith manhattan samplesForAngelica M.empty) `shouldBe` 0.0
            (distanceWith manhattan M.empty samplesForBill) `shouldBe` 0.0
            (distanceWith manhattan samplesForAngelica (M.difference samplesForAngelica samplesForBill)) `shouldBe` 0.0

        it "distanceWith euclidean should result in 0 for no ratings in common" $ do
            (distanceWith euclidean M.empty M.empty) `shouldBe` 0.0
            (distanceWith euclidean samplesForAngelica M.empty) `shouldBe` 0.0
            (distanceWith euclidean M.empty samplesForBill) `shouldBe` 0.0
            (distanceWith euclidean samplesForAngelica (M.difference samplesForAngelica samplesForBill)) `shouldBe` 0.0
